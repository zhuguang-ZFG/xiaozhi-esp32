...
Waveshare 主屏使用 480×320 横向画布。顶部图标区保持透明，状态信息居中放在约 56% 宽的软胶囊中；内容区使用 8px 基础间距和 32px 内边距，Grobot 背后是 468×308 的圆角纸感舞台。预览和机器控制采用全屏半透遮罩 + 居中卡片，避免同时看到背景动作与当前确认动作。

控制抽屉将暂停/继续、重复、试笔和停止置于手动工具之前；点动、设原点、解锁、电机关闭和复位收进「点动·手动」区（2026-08-20 由「高级调试」更名），开机默认折叠、展开态跨抽屉开合保持。抽屉面板可垂直滚动，面板内按钮一律 `CLICKED`——从按钮起手拖成滚动时 LVGL 只发 `PRESS_LOST`、释放不发 `CLICKED`，高危键天然免疫滚屏误触。点动 fail-closed：先发 `?` 取新鲜有限 MPos，再按 X≤190/Y≤277mm 包线判定，取不到新鲜坐标或越界即拒。所有主按钮保持至少 56px 高。
...
### Chips
- **Style:** 状态和通知使用居中软胶囊，深表面背景、低对比边框和正文色；状态颜色同时通过文字表达。
...
### Grobot Stage
Grobot 表情居中、尺寸由设备脸部实现决定，舞台只提供纸感承托；副标题在底部居中胶囊中显示，不能遮挡表情，也不能与预览确认卡竞争。
